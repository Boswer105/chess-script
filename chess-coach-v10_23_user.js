...
(function () {
  ...
  const BATCH_SIZE           = 1;
  const PREC_DEPTH           = 20;
  const NOISE_FLOOR          = 30; // cpLoss < 30 = bruit Stockfish → traité comme coup parfait
  ...
  async function runBatchPrecision(forceAll = false) {
    ...
    for (const entry of ourPending) {
      ...
      if (refBefore && refAfter && refBefore.mate === null && refAfter.mate === null) {
        // cpLoss = score_meilleur_avant - score_après_notre_coup (même point de vue)
        // refBefore.score = meilleur score pour nous avant notre coup
        // refAfter.score  = meilleur score pour l'ennemi après notre coup = -score pour nous
        // score_après(nous) = -refAfter.score
        // cpLoss = max(0, refBefore.score - (-refAfter.score)) = max(0, refBefore.score + refAfter.score)
        let cpLoss;
        if (entry.followedStockfish) {
          cpLoss = 0;
        } else {
          cpLoss = Math.max(0, Math.min(500, refBefore.score + refAfter.score));
          // Absorber le bruit Stockfish : < NOISE_FLOOR = coup parfait
  ...
  function buildPanel() {
    ...
    p.innerHTML = `
      ...
      <div id="cc-precision-section" style="padding:8px 14px 10px;">
        ...
        <!-- ── QUALITÉ DES COUPS ── -->
        <div style="margin-top:8px;border-top:1px solid #1e3348;padding-top:8px;">
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:7px;">

            <!-- Nous -->
            <div style="background:rgba(255,255,255,0.03);border:1px solid #1e3348;border-radius:8px;padding:6px;text-align:center;">
              <div style="font-size:9px;color:#7a9ab5;margin-bottom:4px;font-weight:600;text-transform:uppercase;letter-spacing:.4px;">Vous</div>
              <div style="display:flex;justify-content:center;gap:6px;">
                <div style="display:flex;flex-direction:column;align-items:center;">
                  <span id="cc-good-us" style="font-size:14px;font-weight:800;color:#00c878;">0</span>
                  <span style="font-size:8px;color:#7a9ab5;margin-top:1px;">✅ Bons</span>
                </div>
                <div style="width:1px;background:#1e3348;"></div>
                <div style="display:flex;flex-direction:column;align-items:center;">
                  <span id="cc-bad-us" style="font-size:14px;font-weight:800;color:#e05555;">0</span>
                  <span style="font-size:8px;color:#7a9ab5;margin-top:1px;">❌ Mauvais</span>
                </div>
              </div>
            </div>
            ...
            <!-- Adversaire -->
            <div style="background:rgba(255,255,255,0.03);border:1px solid #1e3348;border-radius:8px;padding:6px;text-align:center;">
              <div style="font-size:9px;color:#7a9ab5;margin-bottom:4px;font-weight:600;text-transform:uppercase;letter-spacing:.4px;">Adverse</div>
              <div style="display:flex;justify-content:center;gap:6px;">
                <div style="display:flex;flex-direction:column;align-items:center;">
                  <span id="cc-good-them" style="font-size:14px;font-weight:800;color:#00c878;">0</span>
                  <span style="font-size:8px;color:#7a9ab5;margin-top:1px;">✅ Bons</span>
                </div>
                <div style="width:1px;background:#1e3348;"></div>
                <div style="display:flex;flex-direction:column;align-items:center;">
                  <span id="cc-bad-them" style="font-size:14px;font-weight:800;color:#e05555;">0</span>
                  <span style="font-size:8px;color:#7a9ab5;margin-top:1px;">❌ Mauvais</span>
                </div>
              </div>
            </div>
  ...
  async function scanAndAnalyse() {
    ...
    if (turn === enemyColor) {
      if (fen !== lastEnemyFen) {
        ...
        if (prevOurFen && fullmove > PRECISION_START_MOVE) {
          const playedMove = getLastPlayedMoveUci();
          const followedStockfish = autoPlayOn ||
    ...
    if (prevEnemyFen && fullmove > PRECISION_START_MOVE) {
