...
(function () {
  ...
  function buildPanel() {
    ...
    p.querySelector('#cc-humanlevel').addEventListener('change', e => {
      ...
      const names = { 1: '~1500', 2: '~2000', 3: '~2500' };
  ...
  function analyseLocal(fen, level) {
    ...
    // En mode humain on demande 3 coups pour pouvoir choisir un sous-optimal
    const multipv = (humanModeOn && autoPlayOn) ? 3 : 1;
    return new Promise(resolve => {
      GM_xmlhttpRequest({
        method:'POST', url:SERVER+'/analyse',
        headers:{'Content-Type':'application/json'},
  ...
  async function autoPlayMove(uci) {
    ...
    if (humanModeOn) {
      ...
      // Mode normal : mix de coups rapides et lents
      const r = Math.random();
      if (r < 0.25) {
        // 25% : coup rapide (évident) — 300ms à 900ms
        delay = 300 + Math.random() * 600;
      } else if (r < 0.75) {
        // 50% : coup normal — 900ms à 2500ms
        delay = 900 + Math.random() * 1600;
      } else {
        // 25% : coup après réflexion — 2500ms à 5000ms
